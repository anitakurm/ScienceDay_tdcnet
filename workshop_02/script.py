...
def plot_manual_pred(ax, Y_pred_trans):
    ...
    limits = [
        min(min(Y_actual), min(Y_pred_trans)),
        max(max(Y_actual), max(Y_pred_trans)),
    ]
...
main = wd.VBox(
    [
        wd.HTML(
            '<div style="font-size: 2em; font-weight: bold;display: flex;justify-content: center;padding: 1em">Workshop 02 - Predict Mobile Traffic</div>'
        ),
        wd.HBox(
            [
